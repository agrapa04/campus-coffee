 ...
 * still arrives transitively (here via Flyway and the XML client). This will end once the ecosystem
 * finishes moving to Jackson 3. While both are present Hibernate would pick one itself, and a `Map` written
 * through one mapper could read back differently through the other, so we pin
 * `hibernate.type.json_format_mapper` to our mapper.
 *
 * Always active: the [EventEntity] is always mapped (the `events` table always exists), regardless of the
 * persistence mode. It is the only `jsonb` column, so this affects nothing else.
 */
@Configuration
