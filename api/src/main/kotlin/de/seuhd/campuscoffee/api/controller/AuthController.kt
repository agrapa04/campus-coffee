 ...
 * JWT carrying the subject (login name), the user's roles, and a 15-minute expiry. The resource-server
 * filter then trusts an incoming token by its signature and expiry alone.
 */
    ...
    fun token(
        ...
        val claims =
            JwtClaimsSet
                ...
                // a `roles` claim carrying the bare role names (e.g. ["USER","MODERATOR"]); the resource
                // server's converter maps these back to ROLE_* authorities so Basic and Bearer agree
